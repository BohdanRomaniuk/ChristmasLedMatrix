#define TEXT_DIRECTION 1  // Напрямок тексту: 1 - по горизонталі, 0 - по вертекалі
#define MIRR_V 0          // Дзеркальне відображення по вертикалі (0 / 1)
#define MIRR_H 0          // Дзеркальне відображення по вертикалі (0 / 1)

#define TEXT_HEIGHT 0     // Висота на якій біжить текст (від низу матриці)
#define LET_WIDTH 5       // Ширина букви (в пікселях)
#define LET_HEIGHT 8      // Висота букви (в пікселях)
#define SPACE 1           // Довжина пробілу (в пікселях)
...
void fillString(String text, uint32_t color)
{
  ...
  if (scrollTimer.isReady())
  {
    ...
    offset--;
    if (offset < -j * (LET_WIDTH + SPACE))//текст закінчився
    {
      offset = WIDTH + 3;
      fullTextFlag = true;
    }
